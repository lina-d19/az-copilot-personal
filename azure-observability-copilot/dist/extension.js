...
User query: "${userQuery}"`;
  ...
  if (!params.update_timestamp && /update/i.test(userQuery) && (params.start_date || params.date)) {
...
async function generateNaturalLanguageResponse(userQuery, backendData, azureApiKey, endpoint, deployment, apiVersion = "2025-01-01-preview") {
...
async function handleUserQuery(userQuery, azureApiKey, endpoint, deployment) {
  ...
  if (params.function === "get_latency" && /highest latency|most latency|which region has the highest latency|region with the highest latency/i.test(userQuery)) {
    params.compare = true;
  }
  if (params.function === "get_latency" && /(most recent latency|latest latency|current latency|recent latency|latency right now)/i.test(userQuery)) {
    params.date = "recent";
  }
  if (!params.function) {
...
function activate(context) {
  ...
  const handler = async (request, context2, stream, token) => {
